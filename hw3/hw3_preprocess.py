...
def cross_validate(data, labels, kfolds, n):
    ...
    for ifold in range(kfolds):
        ...
        training_labels = np.append(labels[trbegin:trend], labels[0:trend2])
        test_data = np.append(data[trend:n], data[tebegin:teend])
        test_labels = np.append(labels[trend:n], labels[tebegin:teend])
        # Compute Features
        tf, tokens = unigram_tf(training_data, vectorizer)
        tf_test = unigram_tf_transform(test_data, vectorizer)
